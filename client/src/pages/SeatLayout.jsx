...
const SeatLayout = () => {
  ...
  // we have to get the movie id and date from url so using useParams
  const {id, date} = useParams()
  // we have to store the selected seats, selected time and the show
  ...
  // function that fetch the show data from the given id
  const getShow = async () => {
    try {
  ...
  // we have to execute the above function when component loaded
  useEffect(()=> {
    getShow()
  },[])
        ...
        <div className='mt-5 space-y-1'>
          {
            show.dateTime[date].map((item)=>(
              <div  className={`flex ite items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition
