    ...
    <>
              ...
              <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40
                ...
                <div className='relative rounded-lg overflow-hidden'>
                  ...
                  <div className='text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute
                                  bottom-0 left-0'>
                    <p className='flex items-center gap-1 text-gray-400cd '>
                ...
                <p className='font-medium truncate px-2 pt-1'>{movie.title}</p>
                <p className='text-gray-400 text-sm px-2 pt-1'>{movie.release_date}</p>
              </div>
            ))
          }
        </div>
      ...
      <div className='mt-8'>
        ...
        <div className='inline-flex items-center gap-2 border border-gray-600 px-3 py-2 rounded-md'>
          ...
          <input type="number" value={showPrice} onChange={(e)=> setShowPrice(e.target.value)}
      ...
      <div className='mt-6'>
        ...
        <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
          ...
          <button className='bg-primary/80 text-white px-3 py-2 text-sm rounded-lg hover:bg-primary
                  cursor-pointer'
                    ...
                    times.map((time) => (
                      <div key={time} className='border border-primary px-2 py-1 flex items-center rounded'>
                        ...
                        <DeleteIcon onClick={()=> handleRemoveTime(date,time)} width={15} className='ml-2
                                    text-red-500 hover:text-shadow-red-700 cursor-pointer'/>
                      </div>
                    ))
                  }
                </div>
