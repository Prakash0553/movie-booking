...
function MovieDetails() {
  ...
  const {shows, axios, getToken, user,faviouriteMovies, fetchFaviouriteShows, image_base_url} = useAppContext()
  ...
  const handleFaviourite = async () => {
    try {
      ...
      const {data} = await axios.post('/api/user/update-favourite', {movieId: id} ,{headers: 
        {Authorization: `Bearer ${await getToken()}`}})
      
      if(data.success){
        await fetchFaviouriteShows()
  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-30 pt-30 md:pt-50'>
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <div className='flex items-center flex-wrap gap-4 mt-4'>
            ...
            <button className='bg-gray-700 p-2.5 transition rounded-full  cursor-pointer active:scale-95'>
              <Heart className={`w-5 h-5 ${faviouriteMovies.find(movie => movie._id === id) ?
